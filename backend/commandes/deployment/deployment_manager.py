...
{device_info['ip']} ansible_user={device_info['username']} ansible_password={device_info['password']} ansible_connection=network_cli ansible_network_os=ios ansible_port={device_info.get('port', 22)}
"""
    ...
    # def create_playbook(self):
    #     """Créer un playbook Ansible YAML qui utilise la variable 'commands'"""
    #     playbook = [{
    #         'name': 'Deploy Network Commands',
    #         'hosts': 'network_devices',
    #         'gather_facts': False,
    ...
    def deploy_commands(self, device_info, commands):
        try:
            ...
            extra_vars = {
                'commands': commands,
                'device_type': device_info.get('type', 'cisco'),  # Default to cisco
            ...
            # Transformer le dict en string pour --extra-vars
            extra_vars_str = yaml.safe_dump(extra_vars, default_flow_style=True).strip().replace('\n', ' ')
            ...
            print("le resultat a la sorite ",process.stdout)
            return {
...
#                 '-i', inventory_path,
#                 playbook_path,
#                 '--extra-vars', f'ansible_python_interpreter={sys.executable} commands={commands} device_type=cisco'
#             ], capture_output=True, text=True)
